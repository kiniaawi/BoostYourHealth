...
const HairNailsSupplementation = ({ onChangeContent }) => {
  ...
  const [issueCategory, setIssueCategory] = useState("Włosy,Paznokcie");
  ...
  const fetchIssues = () => {
    axios
      ...
      .then((response) => {
        ...
        setIssueCategory("Włosy,Paznokcie");
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  const handleSubmitQuestions = () => {
    ...
    axios
      .post(
        "api/SupplHairNailsDiagnosticQuestions/PostHairNailsDiagnQuestions",
        data
      )
      .then((response) => {
  ...
  const handleSeeAdvice = () => {
    ...
    axios
      .post(
        "api/SupplHairNailsDiagnosticQuestions/PostHairNailsSupplAdvice",
        data
      )
      .then((response) => {
        alert(response.data.StatusMessage);
        clearTextArea();
        onChangeContent("hair-nails-suppl-advice");
        navigate("/hair-nails-suppl-advice");
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  return (
    <Box p={2} sx={{ height: "300vh" }}>
      ...
      <Typography variant="h5" sx={{ textAlign: "center", marginBottom: 5 }}>
        <b>Panel Suplementacji - Włosy i Paznokcie</b>
      </Typography>
      <Card>
        <CardHeader
          title={
                  ...
                  {[
                    ...new Set(
                      allSkinIssues
                        .filter(
                          (issue) =>
                            issue.IssueCategory === "Paznokcie" ||
                            issue.IssueCategory === "Włosy"
                        )
...
export default HairNailsSupplementation;
