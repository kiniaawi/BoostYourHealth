...
const DigestiveSystemSupplementation = ({ onChangeContent }) => {
  const [cookies, setCookie, removeCookie] = useCookies([
    "emailCookie",
    "currentPageCookie",
    "userIdCookie",
  ]);
  setCookie("currentPageCookie", "hair-nails-supplementation", { path: "/" });
  const navigate = useNavigate();
  const userId = cookies.userIdCookie;
  const [issueCategory, setIssueCategory] = useState("Układ Pokarmowy");
  ...
  const fetchIssues = () => {
    axios
      ...
      .then((response) => {
        ...
        setIssueCategory("Układ Pokarmowy");
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  const handleSubmitQuestions = () => {
    ...
    axios
      .post(
        "api/SupplDigestSysDiagnosticQuestions/PostDigestSysDiagnQuestions",
        data
      )
      .then((response) => {
  ...
  const handleSeeAdvice = () => {
    ...
    axios
      .post(
        "api/SupplDigestSysDiagnosticQuestions/PostDigestSysSupplAdvice",
        data
      )
      .then((response) => {
        alert(response.data.StatusMessage);
        clearTextArea();
        onChangeContent("digestive-system-suppl-advice");
        navigate("/digestive-system-suppl-advice");
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  return (
    <Box p={2} sx={{ height: "300vh" }}>
      ...
      <Typography variant="h5" sx={{ textAlign: "center", marginBottom: 5 }}>
        <b>Panel Suplementacji - Układ Pokarmowy</b>
      </Typography>
      <Card>
        <CardHeader
          title={
                ...
                <Stack direction="column" sx={{ marginTop: 3, marginLeft: 4 }}>
                  ...
                  {[
                    ...new Set(
                      allSkinIssues
                        .filter(
                          (issue) => issue.IssueCategory === "Układ Pokarmowy"
                        )
...
export default DigestiveSystemSupplementation;
