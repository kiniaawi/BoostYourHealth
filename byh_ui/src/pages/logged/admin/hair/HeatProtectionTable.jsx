...
const HeatProtectionTable = () => {
  ...
  const handleAddFile = () => {
    ...
    axios
      .post("/api/HeatProtection/SaveFile", formData)
      .then((response) => {
        alert(response);
      })
      .catch((error) => {
  ...
  const handleEdit = () => {
    ...
    const data = {
      ...
      HairTypeId: editProduct.HairTypeId,
    ...
    axios
      .put(`/api/HeatProtection/UpdateHeatProtection/${editProduct.Id}`, data)
      .then((response) => {
        fetchProducts();
  ...
  const handleRevert = () => {
    ...
    axios
      .put(`/api/HeatProtection/RevHeatProtection/${selectedProduct.Id}`)
      .then((response) => {
        console.log(selectedProduct);
        fetchProducts();
      })
  ...
  const handleDelete = () => {
    console.log(selectedProduct);
    axios
      .put(`/api/HeatProtection/DelHeatProtection/${selectedProduct.Id}`)
      .then((response) => {
        console.log(selectedProduct);
        fetchProducts();
      })
          ...
          <Grid container spacing={2}>
            <Card>
              <Typography variant="h5" sx={{ textAlign: "center", p: 3 }}>
                <b>Edytuj Produkt Termoochronny </b>
              </Typography>
              <CardContent sx={{ maxHeight: "600px", overflow: "auto" }}>
                <form onSubmit={handleSubmit}>
                  ...
                  >
                    ...
                    <Select
                      value={editProduct.HairTypeId}
                      onChange={(e) =>
                        setEditProduct({
                          ...editProduct,
                          HairTypeId: e.target.value,
                        })
                      }
                      required
                      sx={{ width: "25%" }}
      ...
      <div style={{ overflow: "auto" }}>
        {productsData && productsData.length !== 0 ? (
            ...
            <CardContent>
              <Box>
                <Typography variant="h6" textAlign={"center"} marginBottom={1}>
                  <b>Tabela Produktów Termoochronnych</b>
                </Typography>
              </Box>
          ...
          <Box textAlign={"center"} marginTop={2}>
            <Typography>
              Nie Znaleziono Produktów Termoochronnych w Bazie Danych
            </Typography>
          </Box>
        )}
      </div>
...
export default HeatProtectionTable;
