...
const HairMasksTable = () => {
  ...
  const handleAddFile = () => {
    ...
    axios
      .post("/api/HairMasks/SaveFile", formData)
      .then((response) => {
        alert(response);
      })
      .catch((error) => {
  ...
  const handleEdit = () => {
    ...
    axios
      .put(`/api/HairMasks/UpdateHairMask/${editProduct.Id}`, data)
      .then((response) => {
        fetchProducts();
  ...
  const handleRevert = () => {
    ...
    axios
      .put(`/api/HairMasks/RevHairMask/${selectedProduct.Id}`)
      .then((response) => {
        console.log(selectedProduct);
        fetchProducts();
      })
  ...
  const handleDelete = () => {
    console.log(selectedProduct);
    axios
      .put(`/api/HairMasks/DelHairMask/${selectedProduct.Id}`)
      .then((response) => {
        console.log(selectedProduct);
        fetchProducts();
      })
      ...
      <div style={{ overflow: "auto" }}>
        {productsData && productsData.length !== 0 ? (
          ...
          <Box textAlign={"center"} marginTop={2}>
            <Typography>
              Nie Znaleziono Masek do Włosów w Bazie Danych
            </Typography>
          </Box>
        )}
      </div>
