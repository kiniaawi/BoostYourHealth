...
const HairOilsTable = () => {
  ...
  const handleAddFile = () => {
    ...
    axios
      .post("/api/HairOils/SaveFile", formData)
      .then((response) => {
        alert(response);
      })
      .catch((error) => {
  ...
  const handleEdit = () => {
    ...
    axios
      .put(`/api/HairOils/UpdateHairOil/${editProduct.Id}`, data)
      .then((response) => {
        fetchProducts();
  ...
  const handleRevert = () => {
    ...
    axios
      .put(`/api/HairOils/RevHairOil/${selectedProduct.Id}`)
      .then((response) => {
        console.log(selectedProduct);
        fetchProducts();
      })
  ...
  const handleDelete = () => {
    console.log(selectedProduct);
    axios
      .put(`/api/HairOils/DelHairOil/${selectedProduct.Id}`)
      .then((response) => {
        console.log(selectedProduct);
        fetchProducts();
      })
      ...
      <div style={{ overflow: "auto" }}>
        {productsData && productsData.length !== 0 ? (
          ...
          <Box textAlign={"center"} marginTop={2}>
            <Typography>
              Nie Znaleziono Olejów do Włosów w Bazie Danych
            </Typography>
          </Box>
        )}
      </div>
