...
namespace byh_api.Controllers
{
    ...
    [ApiController]
    public class HairPeelingsController : ControllerBase
    {
        private readonly IConfiguration _configuration;
        private readonly IWebHostEnvironment _env;

        public HairPeelingsController(IConfiguration configuration, IWebHostEnvironment env)
        {
        ...
        public JsonResult Get()
        {
            ...
            try
            {
                string query = @"SELECT * from dbo.HairPeelings";
        ...
        [HttpPost]
        public JsonResult Post(HairPeelings prod)
        {
            Response response = new Response();

            try
            {
                string query = @"INSERT INTO dbo.HairPeelings VALUES(@ProductName, @ProductTypeId, 
        ...
        [HttpPut("UpdateHairPeeling/{Id}")]
        public JsonResult UpdateHairPeeling(HairPeelings prod, int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.HairPeelings SET ProductName = @ProductName, ProductTypeId = @ProductTypeId, 
        ...
        [HttpPut("DelHairPeeling/{Id}")]
        public JsonResult DelHairPeeling(int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.HairPeelings SET IsDeleted = 1
                ...
                using (SqlConnection myConn = new SqlConnection(sqlDataSource))
                {
                    ...
                    response.StatusCode = 200;
                    response.StatusMessage = "HairPeeling Deleted Successfully";
                    HttpContext.Response.StatusCode = response.StatusCode;
                    response.Data = table;
                }
            }
            catch (Exception ex)
            {
                ...
                response.StatusCode = 100;
                response.StatusMessage = "Failed to Delete HairPeeling";
                HttpContext.Response.StatusCode = response.StatusCode;
            }
        ...
        [HttpPut("RevHairPeeling/{Id}")]
        public JsonResult RevHairPeeling(int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.HairPeelings SET IsDeleted = 0
                ...
                using (SqlConnection myConn = new SqlConnection(sqlDataSource))
                {
                    ...
                    response.StatusCode = 200;
                    response.StatusMessage = "HairPeeling Reverted Successfully";
                    HttpContext.Response.StatusCode = response.StatusCode;
                    response.Data = table;
                }
            }
            catch (Exception ex)
            {
                ...
                response.StatusCode = 100;
                response.StatusMessage = "Failed to Revert HairPeeling";
                HttpContext.Response.StatusCode = response.StatusCode;
            }
        ...
        public JsonResult SaveFile()
        {
            ...
            try
            {
                ...
                if (postedFile != null && postedFile.Length > 0)
                {
                    ...
                    var physicalPath = Path.Combine(_env.ContentRootPath, "Photos/HairPeelings", filename);
