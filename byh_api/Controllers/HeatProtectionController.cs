...
namespace byh_api.Controllers
{
    ...
    [ApiController]
    public class HeatProtectionController : ControllerBase
    {
        private readonly IConfiguration _configuration;
        private readonly IWebHostEnvironment _env;

        public HeatProtectionController(IConfiguration configuration, IWebHostEnvironment env)
        {
        ...
        public JsonResult Get()
        {
            ...
            try
            {
                string query = @"SELECT * from dbo.HeatProtection";
        ...
        [HttpPost]
        public JsonResult Post(HeatProtection prod)
        {
            Response response = new Response();

            try
            {
                string query = @"INSERT INTO dbo.HeatProtection VALUES(@ProductName, @ProductTypeId, 
        ...
        [HttpPut("UpdateHeatProtection/{Id}")]
        public JsonResult UpdateHeatProtection(HeatProtection prod, int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.HeatProtection SET ProductName = @ProductName, ProductTypeId = @ProductTypeId, 
        ...
        [HttpPut("DelHeatProtection/{Id}")]
        public JsonResult DelHeatProtection(int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.HeatProtection SET IsDeleted = 1
                ...
                using (SqlConnection myConn = new SqlConnection(sqlDataSource))
                {
                    ...
                    response.StatusCode = 200;
                    response.StatusMessage = "HeatProtection Deleted Successfully";
                    HttpContext.Response.StatusCode = response.StatusCode;
                    response.Data = table;
                }
            }
            catch (Exception ex)
            {
                ...
                response.StatusCode = 100;
                response.StatusMessage = "Failed to Delete HeatProtection";
                HttpContext.Response.StatusCode = response.StatusCode;
            }
        ...
        [HttpPut("RevHeatProtection/{Id}")]
        public JsonResult RevHeatProtection(int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.HeatProtection SET IsDeleted = 0
                ...
                using (SqlConnection myConn = new SqlConnection(sqlDataSource))
                {
                    ...
                    response.StatusCode = 200;
                    response.StatusMessage = "HeatProtection Reverted Successfully";
                    HttpContext.Response.StatusCode = response.StatusCode;
                    response.Data = table;
                }
            }
            catch (Exception ex)
            {
                ...
                response.StatusCode = 100;
                response.StatusMessage = "Failed to Revert HeatProtection";
                HttpContext.Response.StatusCode = response.StatusCode;
            }
        ...
        public JsonResult SaveFile()
        {
            ...
            try
            {
                ...
                if (postedFile != null && postedFile.Length > 0)
                {
                    ...
                    var physicalPath = Path.Combine(_env.ContentRootPath, "Photos/HeatProtection", filename);
